...
%        Y         Data                                 (nobs x n)  
% OUTPUT 
%        S.Am       Predicted state vector  mu_t|t-1      (nobs x m)  
%        S.AmU      Filtered  state vector  mu_t|t        (nobs+1 x m)  
%        S.Pm       Predicted covariance of mu_t|t-1      (nobs x m x m)  
%        S.PmU      Filtered  covariance of mu_t|t        (nobs+1 x m x m)  
...
% Output structure & dimensions
  ...
  S.Am  = nan(m,nobs);  %stores Predicted state vector  mu_t|t-1  
  S.Pm  = nan(m,m,nobs); %stores Predicted covariance of mu_t|t-1        
  S.AmU = nan(m,nobs);   %stores Filtered  state vector  mu_t|t
  S.PmU = nan(m,m,nobs); %stores Filtered  covariance of mu_t|t
  S.ZF = cell(nobs);     
  S.V = cell(nobs);      
  %______________________________________________________________________
