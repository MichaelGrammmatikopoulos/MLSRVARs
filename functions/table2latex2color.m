...
% ----------------------------------------------------------------------- %
function [filef] = table2latex2color(T, filename, score_i, tbl_title, tbl_subtitle, table_id, all_models, fieldNames)
    ...
    for c = 2:n_col
        ...
        horizon_row = [horizon_row '& h = ' string(fieldNames{1})];
        for jj = 2:length(fieldNames)
            horizon_row = [horizon_row '& h = ' string(fieldNames{jj})];
        end
        % horizon_row = [horizon_row '& h = ' string(fieldNames{1}) ' & h = ' fieldNames{2} ' & h = ' fieldNames{3}];
    end
    unique_vars = strrep(strrep(strrep(string(T.Properties.VariableNames),string(fieldNames{1}) ,''),string(fieldNames{2}) ,''),string(fieldNames{3}) ,'');
    ...
    fileID = fopen(filename, 'w');
        fprintf(fileID, '\\begin{flushleft} \n', col_spec);
        fprintf(fileID, '\\begin{table} \n', col_spec);
        % fprintf(fileID, '\\vspace{-3.5cm} \n', col_spec);
        fprintf(fileID, '\\caption{%s} \n', tbl_title);
        fprintf(fileID, '\\parbox{\\textwidth}{ \n', tbl_title);
        fprintf(fileID, '\\centering \n', tbl_title);
        fprintf(fileID, '\\begin{adjustbox}{tabular = %s,scale = {0.55}, center=\\textwidth+2.6cm}\n', col_spec);
        fprintf(fileID, '\\rotatebox{0}{\\textbf{\\large{%s}}} \n', char(string(score_i)));
        fprintf(fileID, '%s \\\\ \n', horizon_row);
        if table_id==2
        ...
        try
            for row = 1:size(T,1)
                ...
                % end
                % 1. Get the content out of the cell (which might be a string or char array)
                
                temp = cell(1, n_col);
                for col = 1:n_col
                    if col>1
                        cell_content = T{row,col};
                        myCharArray = char(cell_content);
                        substring1 = myCharArray(1:4);
                        substring2 = myCharArray(5:length(myCharArray));
                        value = append('\mygraded{',substring1,'}',substring2);
                    else
                        value = T{row,col};
                    end
